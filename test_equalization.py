...
dfe_n                = int(os.getenv('DFE_N', 10))   # Pre Taps
...
group_delay_fudge_factor = 2 # so that the sampling point is at about 0, samples_per_symbol, ... of filtered_trunc. Could fractionally interpolate but no need for now
...
s = signal.lfilter(b, a, np.hstack((np.ones(1), np.zeros(1000))))[230:300]
x = np.linspace(230, 230+len(s)-1, len(s))
main_x = np.argmax(s)
